...
namespace PMSTest
{
    ...
    public static class DataBuilderExtensions
    {
        ...
        {
            var types = assembly.Object.GetTypes();

            if (types == null)
            {
                types = new IType[] { type.Object };
            }
            else
            {
                var newTypes = types.ToList();
                newTypes.Add(type.Object);
                types = newTypes.ToArray();
            }
            assembly.Setup(a => a.GetTypes()).Returns(types);
            return assembly;
        }

        public static Mock<IType> With(this Mock<IType> type, Mock<IMethodInfo> method)
        {
            var methods = type.Object.GetMethods();

            if (methods == null)
            {
                methods = new IMethodInfo[] { method.Object };
            }
            else
            {
                var newTypes = methods.ToList();
                newTypes.Add(method.Object);
                methods = newTypes.ToArray();
            }

            type.Setup(t => t.GetMethods()).Returns(methods);
            return type;
        }

        public static Mock<IMethodInfo> With(this Mock<IMethodInfo> method, Type attribute)
        {
            method.Setup(t => t.GetCustomAttributes(attribute, It.IsAny<bool>())).Returns(new object[] { Activator.CreateInstance(attribute) });
            return method;
        }
    }
